...
import nightknight.collision.geometry.GeometryUtil;
import nightknight.collision.geometry.SearchRectangleObject;
...
public class QuadTree {
    ...
    public void update(RectangleObject initialRectangleObject, RectangleObject updatedRectangleObject){
        ...
        for(int i = 0; i < quadTree.getRectangleObjects().size(); i++){
            if(quadTree.getRectangleObjects().get(i).getId().equals(initialRectangleObject.getId())){
            	...
            	rectangleObjectToUpdate.setWidth(updatedRectangleObject.getWidth());
            	rectangleObjectToUpdate.setHeight(updatedRectangleObject.getHeight());
            	
            	//if the updated properties still lie in this quadtree, just add the object to the list
            	if(GeometryUtil.rectangleObjectIsInside(new SearchRectangleObject(quadTree.getX(), quadTree.getY(), quadTree.getW(), quadTree.getH()),updatedRectangleObject)){
            		...
            		while(quadTree.getParent() != null && !GeometryUtil.rectangleObjectIsInside(new SearchRectangleObject(quadTree.getX(), quadTree.getY(), quadTree.getW(), quadTree.getH()), rectangleObjectToUpdate)){
            			quadTree = quadTree.getParent();
            		}
            		//only insert if it actually fits in the quadtree I am on
            		if(GeometryUtil.rectangleObjectIsInside(new SearchRectangleObject(quadTree.getX(), quadTree.getY(), quadTree.getW(), quadTree.getH()), rectangleObjectToUpdate)){
                		quadTree.insert(rectangleObjectToUpdate);
            		}
    ...
    public List<RectangleObject> search(RectangleObject rectangleObject){
        ...
        ListIterator<RectangleObject> iterator = search(new ArrayList<>(), rectangleObject).listIterator();
    ...
    public List<RectangleObject> collisions(RectangleObject rectangleObject){
        List<RectangleObject> returnList = new ArrayList<>();
        //here I will need to filter through these and only return the objects that are in the search area (even if they are partially in the search area)
        ListIterator<RectangleObject> iterator = search(rectangleObject).listIterator();
                
        while(iterator.hasNext()){
            RectangleObject currentRectangleObject = iterator.next();
            if(currentRectangleObject.overlap(rectangleObject)){
                returnList.add(currentRectangleObject);
            }
        }
        ...
        if(index == QuadTree.THIS_QUADTREE || this.children[0] == null){
            ...
            if(this.children[0] != null){
                ...
                for(int i = 0; i < this.children.length; i++){
                    if(GeometryUtil.rectangleObjectsOverlap(new SearchRectangleObject(this.children[i].getX(), this.children[i].getY(), this.children[i].getW(), this.children[i].getH()), rectangleObject)) {
    ...
    protected int getChildIndexRectangleBelongsIn(RectangleObject rectangleObject){
        ...
        boolean fitsCompletelyInNorthHalf = rectangleObject.getY() < horizontalDividingLine && (rectangleObject.getHeight()+ rectangleObject.getY() < horizontalDividingLine);
        boolean fitsCompletelyInSouthHalf = rectangleObject.getY() > horizontalDividingLine;
        boolean fitsCompletelyInWestHalf = rectangleObject.getX() < verticalDividingLine && (rectangleObject.getX() + rectangleObject.getWidth() < verticalDividingLine);
